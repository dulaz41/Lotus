...
const Footer: React.FC<HeroProps> = () => {
  const [isConnected, setIsConnected] = useState<boolean | any>(false);

  const { address }: any = useAccount()
  // Update the connected state when the account changes
  useEffect(() => {
    if (address) {
      setIsConnected(true);
    } else {
  ...
  useEffect(() => {
    ...
    let currentIndex = 0;
    const words = ["Ready to kickstart your project? Share your ingenious concepts with Lotus for an opportunity to secure funding and backing within the blockchain ecosystem.", "Your innovative ideas could pave the way for substantial support and advancement in this dynamic space."];
  ...
  return (
    <footer>
      <div className='mt-[35px]  mb-[20px] mx-auto max-w-6xl'>
        ...
        <div className=' lg:pt-12 lg:pb-8  lg:bg-cover flex-shrink-0  lg:bg-no-repeat' style={{ backgroundImage: `url('/images/Footer.png')` }}>
          <div className="mx-auto py-1 lg:px-8">
            ...
            >
              As a project creator, submit your innovative ideas to OP Grant for a chance to receive funding and support for your project within the OP ecosystem.
            </motion.h1> */}
              <AnimatePresence>
                <motion.span
