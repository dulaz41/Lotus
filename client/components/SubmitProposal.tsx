...
const SubmitProposal: React.FC = () => {
  ...
  const { signMessage }: any = useSignMessage();
  ...
  const signMessenger = async (): Promise<string | undefined> => {
    const MSG = Buffer.from(`Creating Proposal for ${address}`).toString("hex");
    try {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Sign the message
    const signature = await signMessenger();
    ...
    if (window.ethereum)
      try {
        ...
        // console.log(signers)
        const contractAddr = "0xA52A2fB8457F8BEb875a4721a24F590580f86964";
